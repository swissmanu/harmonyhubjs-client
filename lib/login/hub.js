...
function getIdentity(userAuthToken, hubhost, hubport) {
	...
	xmppClient.on('stanza', function(stanza) {
		...
		if(stanza.getChild('oa')) {
			...
			// There is not only one stanza recieved. How to handle the first
			// one, which we are not interested into actually?
			//callback(new Error('Retrieving session token failed. Hub response not as expected.'));
		}
	});
