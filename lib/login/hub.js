 ...
 *
 * Parameters:
 *     (String) userAuthToken - A authentication token, retrieved from logitech's
 *                              web service.
...
function getIdentity(userAuthToken, hubhost, hubport) {
	...
	xmppClient.on('online', function() {
		...
		var body = 'token=' + userAuthToken + ':name=harmonyjs#iOS6.0.1#iPhone'
	...
	xmppClient.on('stanza', function(stanza) {
		...
		if(stanza.attrs.id === iqId.toString()) {
			var body = stanza.getChildText('oa')
				, response = util.decodeXmppResponse(body);
...
function loginToHub(userAuthToken, hubhost, hubport) {
	debug('perform hub login using auth token: ' + userAuthToken);
